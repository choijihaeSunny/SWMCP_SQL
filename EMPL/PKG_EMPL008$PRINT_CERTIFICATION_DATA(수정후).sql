...
begin
	...
	CALL USP_SYS_GET_ERRORINFO(V_RETURN); 
  	SELECT a.comp_id as COMP_ID,
             ...
             (
             	case 
	             	when a.CERT_KIND = '1' then '재  직  증  명  서'
	             	when a.CERT_KIND = '2' then '경  력  증  명  서'
	             	else '퇴  직  증  명  서' -- a.CERT_KIND = '3'
	            END
             ) as CERT_KIND,
             str_to_date(A.REQ_DATE,'%Y%m%d') AS APPLY_DATE,   
             str_to_date(A.BAL_DATE,'%Y%m%d')  AS PASSPOST_DATE,   
             a.BAL_NO   AS PASSPOST_NO,   
             CONCAT(b.kor_name,if(B.CHN_NAME != '' or B.CHN_NAME is not null,concat('(',B.CHN_NAME,')'),''))      AS  KOR_NAME,
             ...
             -- if(B.RETIRE_DATE = '' or B.RETIRE_DATE IS null, str_to_date(A.BAL_DATE,'%Y%m%d'), str_to_date(B.RETIRE_DATE,'%Y%m%d')) AS RETIRE_DATE,
             if(B.RETIRE_DATE = '' or B.RETIRE_DATE IS null, SYSDATE(), str_to_date(B.RETIRE_DATE,'%Y%m%d')) AS RETIRE_DATE,
--              year(if(B.RETIRE_DATE = '' or B.RETIRE_DATE is null,str_to_date(A.BAL_DATE,'%Y%m%d'),str_to_date(B.RETIRE_DATE,'%Y%m%d'))) - year(str_to_date(B.ENTER_DATE,'%Y%m%d')) as TERM_YEAR,
--              mod(timestampdiff(month,str_to_date(B.ENTER_DATE,'%Y%m%d'),if(B.RETIRE_DATE = '' or B.RETIRE_DATE is null,str_to_date(A.BAL_DATE,'%Y%m%d'),str_to_date(B.RETIRE_DATE,'%Y%m%d'))),12) as TERM_MONTH,
             year(SYSDATE()) - year(str_to_date(B.ENTER_DATE,'%Y%m%d')) as TERM_YEAR, -- 근속년수
             mod(timestampdiff(month, str_to_date(B.ENTER_DATE,'%Y%m%d'), SYSDATE()),12) as TERM_MONTH,
...
--                                         else '위 사람은 상기의 경력이 틀림없음을 증명합니다.' end TEX_CONTS
             (
             	case 
	             	when a.CERT_KIND = '1' then '재직하고 있음을 증명합니다.'
	             	when a.CERT_KIND = '2' then '상기의 경력이 틀림없음을 증명합니다'
	             	else '재직하였음을 증명합니다.' -- a.CERT_KIND = '3'
	            END
             ) as TEX_CONTS
