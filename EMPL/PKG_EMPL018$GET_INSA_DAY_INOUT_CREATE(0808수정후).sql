...
PROC:BEGIN
   ...
   update INSA_DAY_INOUT
      set DELETE_YN = 'Y'
          ,MODI_KEY = MODI_KEY + 1
          ,UPDATE_ID = A_SYS_USER
          ,UPDATE_DT = SYSDATE()
		  ...
		  Else
	       ...
	       if A_RST_TIME = '' or  A_REND_TIME = '' THEN
			  ...
			  SELECT  	A_COMP_ID,
						...
						T_A.ST_TIME,
						T_A.END_TIME,
						case when  SUBSTRING(T_A.R_ST_TIME,12,8) = '08:00:00' and   SUBSTRING(T_A.R_END_TIME,12,8) = '18:00:00' then 8 else  FN_EMP_WORK_TIME_CAL(T_A.COMP_ID, 'A', STR_TO_DATE(T_A.S_ST_TIME, '%Y-%m-%d %H:%i:%s'),  STR_TO_DATE(T_A.S_END_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_ST_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_END_TIME, '%Y-%m-%d %H:%i:%s')) end  as WORK_TIME,
						case when  SUBSTRING(T_A.R_ST_TIME,12,8) = '08:00:00' and   SUBSTRING(T_A.R_END_TIME,12,8) = '18:00:00' then 0 else  FN_EMP_WORK_TIME_CAL(T_A.COMP_ID, 'B', STR_TO_DATE(T_A.S_ST_TIME, '%Y-%m-%d %H:%i:%s'),  STR_TO_DATE(T_A.S_END_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_ST_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_END_TIME, '%Y-%m-%d %H:%i:%s')) end  as EX_TIME,						
						case when  SUBSTRING(T_A.R_ST_TIME,12,8) = '08:00:00' or   SUBSTRING(T_A.R_END_TIME,12,8) = '18:00:00' then 0 else  FN_EMP_WORK_TIME_CAL(T_A.COMP_ID, 'C', STR_TO_DATE(T_A.N_ST_TIME, '%Y-%m-%d %H:%i:%s'),  STR_TO_DATE(T_A.N_END_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_ST_TIME, '%Y-%m-%d %H:%i:%s'), STR_TO_DATE(T_A.R_END_TIME, '%Y-%m-%d %H:%i:%s')) end  as SP_TIME,
						  null,
						T_A.ADJ_EMP,
						T_A.ADJ_DATE,
						0,
				  ...
				  FROM ( select A.COMP_ID,
								 ...
								 date_format(concat( A.SET_DATE,SUBSTRING(REPLACE(B.ST_TIME , ':',''), 12, 6)), '%Y-%m-%d %H:%i:%s')  as R_ST_TIME,
								 date_format(case when convert(SUBSTRING(B.ST_TIME,1,2),INT) < convert(SUBSTRING(B.END_TIME,1,2),INT) then date_format(concat( A.SET_DATE,SUBSTRING(REPLACE(B.END_TIME , ':',''), 12, 6)), '%Y-%m-%d %H:%i:%s')  
								                                  else  /*DATE_ADD(*/ date_format(concat( A.SET_DATE,SUBSTRING(REPLACE(B.END_TIME , ':',''), 12, 6)), '%Y-%m-%d %H:%i:%s')/*, interval 1 day)*/ end, '%Y-%m-%d %H:%i:%s')   as R_END_TIME,
