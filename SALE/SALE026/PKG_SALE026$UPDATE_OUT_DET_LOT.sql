CREATE DEFINER=`ubidom`@`%` PROCEDURE `swmcp`.`PKG_SALE026$UPDATE_OUT_DET_LOT`(	
...
begin
	...
	call SP_SUBUL_CREATE(
		A_COMP_ID, V_SUBUL_KEY, 'UPDATE', date_format(A_SET_DATE,'%Y%m%d'), '2', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO, 100, 
	...
	# 선점수불  1:선점재고입고 , 2:선점재고출고 , 3:선점재고 출고예정차감 , 4:선점재고 출고예정    3번처리하고 2번 처리해야 재고부족오류 발생 안함 
	call SP_SUBUL_ORDER_CREATE(
        A_COMP_ID, concat(V_SUBUL_KEY, '-3'), 'UPDATE', date_format(A_SET_DATE,'%Y%m%d'), '3', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO,
		V_IO_GUBN_OUT, A_QTY, V_TABLE_NAME, A_OUT_KEY, A_CUST_CODE, A_ORDER_KEY,
		A_SYS_ID, A_SYS_EMP_NO, 
		N_RETURN, V_RETURN);
    ...
    #선점 수불 1:선점재고입고 , 2:선점재고출고 , 3:선점재고 출고예정차감 , 4:선점재고 출고예정
	call SP_SUBUL_ORDER_CREATE(
        A_COMP_ID, concat(V_SUBUL_KEY, '-2'), 'UPDATE', date_format(A_SET_DATE,'%Y%m%d'), '2', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO,
		V_IO_GUBN_PRE, A_QTY, V_TABLE_NAME, A_OUT_KEY, A_CUST_CODE, A_ORDER_KEY,
		A_SYS_ID, A_SYS_EMP_NO, 
		N_RETURN, V_RETURN);
