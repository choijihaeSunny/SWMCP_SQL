...
begin
	...
	call SP_SUBUL_CREATE(
		A_COMP_ID, V_SUBUL_KEY, 'DELETE', date_format(A_OUT_DATE,'%Y%m%d'), '2', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO, 100, 
	...
	#선점 수불  # 1:선점재고입고 , 2:선점재고출고 , 3:선점재고 출고예정차감 , 4:선점재고 출고예정  삭제시에는   2번처리하고 3번 처리해야 재고부족오류 발생 안함 
	call SP_SUBUL_ORDER_CREATE(
        A_COMP_ID, concat(V_SUBUL_KEY, '-2'), 'DELETE', date_format(A_OUT_DATE,'%Y%m%d'), '2', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO,
	...
	call SP_SUBUL_ORDER_CREATE(
        A_COMP_ID, concat(V_SUBUL_KEY, '-3'), 'DELETE', date_format(A_OUT_DATE,'%Y%m%d'), '3', V_WARE_CODE, V_ITEM_KIND, A_ITEM_CODE, A_LOT_NO,
