...
begin
	...
	from TB_COATING_PLAN A
		left outer join (
			...
			from   (
					select COMP_ID, CONCAT(YYYY, MM, DD) as CAL_DATE , row_number() OVER(order by CONCAT(YYYY, MM, DD) asc) as CAL_ROWNUM
					from   TC_FACT_CAL
		...
		inner join TB_ITEM_CODE E on D.ITEM_CODE = E.ITEM_CODE
