CREATE DEFINER=`ubidom`@`%` PROCEDURE `swmcp`.`PKG_PURC120$INSERT_OUTSIDE_END_MST`(		
	IN A_COMP_ID VARCHAR(10),
	IN A_CHK VARCHAR(1),
	IN A_SET_DATE TIMESTAMP,
	INOUT A_OUTSIDE_END_KEY VARCHAR(30),
...
begin
	...
	declare V_OUTSIDE_END_KEY VARCHAR(30);
	...
	SET N_RETURN = 0;
  	...
  	if A_CHK = 'Y' then
	  	...
	  	set V_OUTSIDE_END_KEY = CONCAT('EX', V_SET_DATE, V_SET_SEQ);
	  
	    insert into TB_OUTSIDE_END (
	    	COMP_ID,
	    	SET_DATE,
	    	SET_SEQ,
	    	OUTSIDE_END_KEY,
	    ...
	    set A_OUTSIDE_END_KEY = V_OUTSIDE_END_KEY;
