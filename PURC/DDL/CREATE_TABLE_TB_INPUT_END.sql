-- swmcp.tb_input_end definition

CREATE TABLE `tb_input_end` (
  `COMP_ID` varchar(10) NOT NULL COMMENT '사업장코드',
  `SET_DATE` varchar(8) NOT NULL COMMENT '발행일자',
  `SET_SEQ` varchar(4) NOT NULL COMMENT '발행순번',
  `INPUT_END_KEY` varchar(30) NOT NULL COMMENT '입고마감 MST KEY, PX + YYMM + 순번(4)',
  ...
  PRIMARY KEY (`COMP_ID`,`INPUT_END_KEY`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='구매입고마감MST';